...
function uploadProfileImage(imageUrl, id) {
    return new Promise((resolve, reject) => {
        db.query(`UPDATE athletes SET image_url = $1 WHERE id = $2 RETURNING image_url;`, [(process.env.NODE_ENV == 'production' ? 'https://s3.amazonaws.com/stravita/' : '/uploads/') + imageUrl, id]).then((results) => {
            if (results.rows[0] == undefined) {
